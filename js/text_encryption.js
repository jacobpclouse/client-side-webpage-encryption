...
async function mainBody(inputText,inputKey){
  ...
  var files = [
    { name: "encrypted_text", content: returned, extension: "txt" },
    { name: "password_key", content: inputKey, extension: "key" },
    { name: "iv_hash", content: iv_hash, extension: "hash" },
    {
      name: "instructions",
      content:
        "You have downloaded your encrypted text along with the password you used and a random intialization vector called iv_hash. To decrypt this go to the decryption section of jacobclouse.org and upload the encrypted data, the password and the hash files. Then you can submit them and your message will be decrypted!",
      extension: "txt",
    },
  ];

  downloadZip(files, "Text");
...
async function encryptionFunc(msn, enck, h0) {
  ...
  for (let msn_block_no = 0; msn_block_no < msn_chunks.length; msn_block_no++) {
    ...
    for (let i = 0; i < current_block.length; i++) {
      cipher_block += String.fromCharCode(
...
async function decryptionFunc(blk, enck, h0) {
  ...
  let blocks = [];
  for (
    let ciphertext_block = 1;
    ciphertext_block < blk.length;
    ciphertext_block++
  ) {
